...
namespace FreeChat.Views.CustomControls
{
    ...
    public partial class MessageBubble : PancakeView
    {
        ...
        #region ImageTappedCommand
        public static readonly BindableProperty ImageTappedCommandProperty = BindableProperty.Create(nameof(ImageTappedCommand), typeof(ICommand), typeof(MessageBubble), propertyChanged: (obj, old, newV) =>
        {
            var me = obj as MessageBubble;
            if (newV != null && !(newV is ICommand)) return;
            var oldReplyTappedCommand = (ICommand)old;
            var newReplyTappedCommand = (ICommand)newV;
            me?.ImageTappedCommandChanged(oldReplyTappedCommand, newReplyTappedCommand);
        });

        private void ImageTappedCommandChanged(ICommand oldReplyTappedCommand, ICommand newReplyTappedCommand)
        {
